    ...
    if 1 < 3:  # try: # pass on KeyboardInterrupt
        ...
        while True:  # restart loop
            ...
            if not bipop:
                ...
                sigma_factor = 0.01 ** pymoo.PymooPRNG().uniform()  # Local search
                popsize_multiplier = fmin_options['incpopsize'] ** (irun - runs_with_small)
                opts['popsize'] = np.floor(popsize0 * popsize_multiplier ** (pymoo.PymooPRNG().uniform() ** 2))
            ...
            if 1 < 3:
                while not es.stop():  # iteration loop
                    ...
                    if 11 < 3 and opts['vv']:  # inject a solution
                        ...
                        if 0 * pymoo.PymooPRNG().randn() >= 0:
                            X[0] = 0 + opts['vv'] * es.sigma ** 0 * pymoo.PymooPRNG().randn(es.N)
                            fit[0] = yield X[0]
                            # print fit[0]
