...
def de_differential(X, F, dither=None, jitter=True, gamma=0.0001, return_differentials=False):
    ...
    for i, j in pairs:
        ...
        if dither == "vector":
            F = (F + pymoo.PymooPRNG().random(n_matings) * (1 - F))
        elif dither == "scalar":
            F = F + pymoo.PymooPRNG().random() * (1 - F)

        # http://www.cs.ndsu.nodak.edu/~siludwig/Publish/papers/SSCI20141.pdf
        if jitter:
            F = (F * (1 + gamma * (pymoo.PymooPRNG().random(n_matings) - 0.5)))
...
def rnd_F(m):
    return 0.5 * (1 + pymoo.PymooPRNG().uniform(size=len(m)))
