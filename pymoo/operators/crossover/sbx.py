...
def cross_sbx(X, xl, xu, eta, prob_var, prob_bin, eps=1.0e-14):
    ...
    cross = PYMOO_PRNG.random((n_matings, n_var)) < prob_var
    ...
    rand = PYMOO_PRNG.random(len(eta))
    ...
    b = PYMOO_PRNG.random(len(prob_bin)) < prob_bin
    tmp = np.copy(c1[b])
    c1[b] = c2[b]
    c2[b] = tmp
