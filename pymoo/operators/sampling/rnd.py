...
def random(problem, n_samples=1):
    X = pymoo.PymooPRNG().random((n_samples, problem.n_var))
...
class BinaryRandomSampling(Sampling):
    ...
    def _do(self, problem, n_samples, **kwargs):
        val = pymoo.PymooPRNG().random((n_samples, problem.n_var))
...
class IntegerRandomSampling(FloatRandomSampling):
    ...
    def _do(self, problem, n_samples, **kwargs):
        ...
        return np.column_stack([pymoo.PymooPRNG().integers(xl[k], xu[k] + 1, size=n_samples) for k in range(n)])
...
class PermutationRandomSampling(Sampling):
    ...
    def _do(self, problem, n_samples, **kwargs):
        ...
        for i in range(n_samples):
            X[i, :] = pymoo.PymooPRNG().permutation(problem.n_var)
        return X
