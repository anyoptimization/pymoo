...
class SingleObjectiveSpaceTermination(DeltaToleranceTermination):
    ...
    def __init__(self, tol=1e-6, **kwargs) -> None:
...
class MultiObjectiveSpaceTermination(DeltaToleranceTermination):
    ...
    def __init__(self, tol=0.0025, **kwargs):
        super().__init__(tol, **kwargs)

    def _data(self, algorithm):
        F = algorithm.opt.get("F")
        return dict(ideal=F.min(axis=0), nadir=F.max(axis=0), F=F)
    ...
    def _delta(self, prev, current):
        ...
        l_N = normalize(prev["F"], c_ideal, c_nadir)
...
# class MultiObjectiveSpaceToleranceTerminationWithRenormalization(MultiObjectiveSpaceTermination):
#
#     def __init__(self,
#                  n_last=30,
