...
class MW(Problem):
    ...
    def g3(self, X):
        contrib = 2.0 * np.power(X[:, self.n_obj-1:] + (X[:, self.n_obj-2:-1] - 0.5)*(X[:, self.n_obj-2:-1] - 0.5) - 1.0, 2.0)
        distance = 1 + contrib.sum(axis=1)
        return distance
...
class MW6(MW):
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        g0 = f0**2 / np.power(1.0 + self.LA3(0.15, 6.0, 4.0, 10.0, atan), 2.0) + f1**2 / np.power(1.0 + self.LA3(0.75, 6.0, 4.0, 10.0, atan), 2.0) - 1
        out["F"] = np.column_stack([f0, f1])
        out["G"] = g0.reshape((-1, 1))
...
class MW8(MW):
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        g0 = f_squared - (1.25 - self.LA2(0.5, 6.0, 1.0, 2.0, np.arcsin(f[:, -1] / np.sqrt(f_squared))))*(1.25 - self.LA2(0.5, 6.0, 1.0, 2.0, np.arcsin(f[:, -1] / np.sqrt(f_squared))))
        out["F"] = f
        out["G"] = g0.reshape((-1, 1))
...
class MW12(MW):
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        g0 = -1.0*(1 - 0.625*f0 - f1 + 0.08*np.sin(2 * np.pi*(f1 - f0 / 1.6)))*(1.4 - 0.875*f0 - f1 + 0.08*np.sin(2 * np.pi*(f1/1.4 - f0 / 1.6)))
        g1 = (1 - 0.8*f0 - f1 + 0.08*np.sin(2 * np.pi*(f1 - f0 / 1.5)))*(1.8 - 1.125*f0 - f1 + 0.08*np.sin(2 * np.pi*(f1/1.8 - f0 / 1.6)))
        out["F"] = np.column_stack([f0, f1])
        out["G"] = np.column_stack([g0, g1])
    ...
    def _calc_pareto_front(self, ref_dirs=None):
        ...
        invalid = (1-0.8*F[:, 0]-F[:, 1]+0.08*np.sin(2*np.pi*(F[:, 1]-F[:, 0]/1.5)))*(1.8-1.125*F[:, 0]-F[:, 1]+0.08*np.sin(2*np.pi*(F[:, 1]/1.8-F[:, 0]/1.6))) > 0
        while invalid.any():
            F[invalid, :] *= 1.001
            invalid = (1-0.8*F[:, 0]-F[:, 1]+0.08*np.sin(2*np.pi*(F[:, 1]-F[:, 0]/1.5)))*(1.8-1.125*F[:, 0]-F[:, 1]+0.08*np.sin(2*np.pi*(F[:, 1]/1.8-F[:, 0]/1.6))) > 0
        return F
...
class MW13(MW):
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        g0 = -1.0*(5.0 - (1 + f0 + 0.5*f0 * f0) - 0.5*np.sin(3 * np.pi*f0) - f1)*(5.0 - (1 + 0.7*f0) - 0.5*np.sin(3 * np.pi*f0) - f1)
        g1 = (5.0 - np.exp(f0) - 0.5*np.sin(3 * np.pi*f0) - f1)*(5.0 - (1 + 0.4*f0) - 0.5*np.sin(3 * np.pi*f0) - f1)
        out["F"] = np.column_stack([f0, f1])
        out["G"] = np.column_stack([g0, g1])
