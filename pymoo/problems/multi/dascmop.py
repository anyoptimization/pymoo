...
class DASCMOP(Problem):
    ...
    def g3(self, X):
        i = np.arange(self.n_obj-1, self.n_var)
        contrib = (X[:, self.n_obj-1:] - np.cos(0.25*i/self.n_var*np.pi*(X[:, 0:1] + X[:, 1:2])))**2
        return contrib.sum(axis=1).reshape((-1, 1))
...
class DASCMOP8(DASCMOP7):
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        cos_x1 = np.cos(0.5*np.pi*X[:, 0:1])
        f0 = cos_x1 + np.cos(0.5*np.pi*X[:, 1:2]) + g
        f1 = cos_x1 + np.sin(0.5*np.pi*X[:, 1:2]) + g
        f2 = np.cos(0.5*np.pi*X[:, 0:1]) + g
