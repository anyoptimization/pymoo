...
class Ackley(Problem):
    ...
    def __init__(self, n_var: int = 2, a: float = 20, b: float = 1 / 5, c: float = 2 * np.pi) -> None:
    ...
    def _evaluate(self, x: Any, out: Any, *args: Any, **kwargs: Any) -> Any:  # noqa: ARG002
        part1 = -1.0 * self.a * anp.exp(-1.0 * self.b * anp.sqrt((1.0 / self.n_var) * anp.sum(x * x, axis=1)))
