...
def pso_canonical(V, X, P_X, L_X, w, c1, c2):
    ...
    r1, r2 = pymoo.PymooPRNG().random((n_particles, n_var)), pymoo.PymooPRNG().random((n_particles, n_var))
...
def pso_rotation_invariant(V, X, P_X, L_X, inertia, c1, c2):
    ...
    r1 = pymoo.PymooPRNG().random((n_particles, n_var))
    p = X + c1 * r1 * (P_X - X)

    r2 = pymoo.PymooPRNG().random((n_particles, n_var))
    l = X + c2 * r2 * (L_X - X)
...
def alea_sphere(G, radius):
    ...
    x = pymoo.PymooPRNG().normal(size=(n, m))
    l = np.sqrt(np.sum(x ** 2, axis=1, keepdims=True))

    r = pymoo.PymooPRNG().random(size=(n, 1))
    x = r * radius * x / l
    return x + G
...
def get_neighbors(name, N):
    if name == "star":
        ...
        neighbors = []
        for i in range(N):
            vals = pymoo.PymooPRNG().permutation(N)[:K]
