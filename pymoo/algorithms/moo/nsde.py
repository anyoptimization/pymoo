...
class VariantDE(InfillCriterion):
    ...
    def _do(self, problem, pop, n_offsprings, random_state=None, **kwargs):
        ...
        P = self.selection.do(problem, pop, n_offsprings, self.n_total_parents,
                              to_pop=False, random_state=random_state)
        ...
        if self.crossover_variant == "bin":
            M = mut_binomial(n_offsprings, n_var, self.CR, at_least_once=True, random_state=random_state)
        elif self.crossover_variant == "exp":
            M = mut_exp(n_offsprings, n_var, self.CR, at_least_once=True, random_state=random_state)
        else:
...
class NSDE(NSGA2):
    ...
    def __init__(self,
        ...
        sampling : Sampling
            Sampling strategy. Defaults to LHS().
        variant : str
            DE strategy string: "DE/selection/n/crossover".
            selection: 'rand', 'best', 'current-to-best', 'current-to-rand', 'ranked'.
            crossover: 'bin' or 'exp'. Defaults to 'DE/rand/1/bin'.
        CR : float
            Crossover rate in [0, 1]. Defaults to 0.7.
        F : float or (float, float)
            Scale factor(s) in (0, 2]. Defaults to randomized in (0, 1).
        gamma : float
            Jitter deviation. Defaults to 1e-4.
        de_repair : str or callable
            Repair for DE donor vectors: 'bounce-back', 'midway', 'rand-init', 'to-bounds'.
        survival : Survival
            Survival strategy. Defaults to RankAndCrowding().
        """
