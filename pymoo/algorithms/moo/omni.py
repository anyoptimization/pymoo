    ...
    def _do(self, problem, pop, *args, n_survive=None, random_state=None, **kwargs):
        ...
        for k, front in enumerate(fronts):
            ...
            crowding_of_front = calc_omni_crowding_distance(
                F[front, :], X[front, :],
                obj_crowding=self.obj_crowding, var_crowding=self.var_crowding,
            )
...
class OmniOptimizer(GeneticAlgorithm):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        """Omni-Optimizer, a generic evolutionary algorithm for single- and multi-objective,
        single- and multi-global optimization proposed by Deb and Tiwari [1]_.

        It is an NSGA-II based algorithm with three distinctive components:
        ...
        pop_size : int
            The population size.
        delta : float
            The epsilon margin (as a fraction of each objective's range) used for the
            loose dominance. ``delta=0`` recovers the usual Pareto dominance.
        obj_crowding, var_crowding : bool
            Whether to niche in objective and/or variable space. Disabling the
            variable-space niching essentially recovers NSGA-II.
        sampling, selection, crossover, mutation, survival, output
            The operators of the genetic algorithm. They default to the operators
            described in the original paper.

        References
        ----------
        .. [1] K. Deb and S. Tiwari, "Omni-optimizer: A generic evolutionary algorithm for
           single and multi-objective optimization", European Journal of Operational
           Research, 185(3), 2008, pp. 1062-1087.
        """

        if survival is None:
            survival = OmniOptimizerSurvival(delta=delta, obj_crowding=obj_crowding,
                                             var_crowding=var_crowding)
