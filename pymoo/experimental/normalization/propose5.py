...
class ProposeReferenceLineSurvival(Survival):
    ...
    def _do(self, pop, n_survive, D=None, **kwargs):
        ...
        if len(feasible) > 0:
            ...
            self.extreme_points = propose_get_extreme_points(F[non_dominated, :], self.ideal_point,
                                                             extreme_points=self.extreme_points)

            # find the intercepts for normalization and do backup if gaussian elimination fails
            self.intercepts = propose_get_intercepts(self.extreme_points, self.ideal_point, nadir_point, worst_point)
            ...
            niche_of_individuals, dist_to_niche = propose_associate_to_niches(F, self.ref_dirs, self.ideal_point,
                                                                              self.nadir_point)
...
def propose_get_extreme_points(F, ideal_point, extreme_points=None):
...
def propose_get_intercepts(extreme_points, ideal_point, nadir_point, worst_point):
...
def propose_associate_to_niches(F, niches, ideal_point, nadir_point):
