...
def calc_metric(D, F, p=2):
    farthest = D.argmax()
    best = F.argmin()

    # sort by distance to best
    delta_x = D / D[farthest]
    delta_f = (F - F[best, 0]) / (F[farthest, 0] - F[best, 0])

    f = np.column_stack([-delta_x, delta_f])
    z = np.array([-1, 0])
    return ((f - z) ** p).sum(axis=1) ** (1 / p)
...
class NichingSurvival(Survival):
    ...
    def _do(self, problem, pop, n_survive, out=None, algorithm=None, **kwargs):
        ...
        while len(survivors) < n_survive:
            ...
            reassign = np.logical_and(D[select] < dist, F[:, 0] >= F[select, 0])
        ...
        for k, front in enumerate(fronts):
            ...
            # if we have found the splitting front
                ...
                delta_f = (F[front, 0] - F[S, 0]) / (F[front[farthest], 0] - F[S, 0])
                f = np.column_stack([-delta_x, delta_f])
                z = np.array([-1, 0])
                p = 2
