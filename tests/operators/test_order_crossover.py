...
def order_crossover_contributed_no_shift(x1, x2, seq=None):
    ...
    if seq is not None:
        ...
        start, end = np.sort(pymoo.PymooPRNG().choice(len(x1), 2, replace=False))
...
def test_equal_constribution_no_shift():
    for _ in range(100):
        a = pymoo.PymooPRNG().permutation(10)
        b = pymoo.PymooPRNG().permutation(10)

        start, end = np.sort(pymoo.PymooPRNG().choice(len(a), 2, replace=False))
